    ...
    <form class="form-horizontal"  v-else>
      <div class="card-body">
        ...
        <div class="form-group row">
          <div class="col-6 row">
            <label for="inputEmail3" class="col-4 col-form-label"><h6>{{ 'NameAction' | localize }}</h6></label>
            <div class="col-8">
              <input
                type="text"
                class="form-control"
...
export default {
  ...
  methods: {
    ...
    async addAction () {
      try {
        ...
        if (this.mainImageData) {
          this.page.mainImg = await Module.addImg( 'Actions', this.lang, this.mainImageData, this.id, 'main')
        }
        console.log(this.page.mainImg)
        this.page.localImgArr = await Module.addOtherImg( 'Actions', this.lang, this.localImg, this.id )
        console.log(this.page.localImgArr)

        await Module.addInfoById('Actions', this.page, this.lang, this.id)
    ...
    async check (id, counter) {
      if (id != counter) {
        const loadNews = await Module.fetchInfoById('Actions', this.id)
