<template>
  <div class="d-flex justify-content-between p-3 flex-column">
    ...
    <div class="custom-control custom-switch d-flex align-items-center mb-3">
      <input type="checkbox" class="custom-control-input" id="customSwitch2" v-model="active">
      <label class="custom-control-label" for="customSwitch2">ВКЛ</label>
    </div>
    <div v-if="active">
...
export default {
  ...
  methods: {
    ...
    async saveBannerToDB () {
      ...
      this.films = await Module.fetchInfoBanner(this.title, this.banner)
      await Module.addBannerSpeed(this.title, 'bannerNewsSpeed', this.speed)
      this.loading = false
    }
  },
  ...
  async mounted () {
    this.films = await Module.fetchInfoBanner(this.title, this.banner)
    this.speed = await Module.fetchInfoBannerSpeed(this.title, 'bannerNewsSpeed')
    this.loading = false
  }
}
</script>
